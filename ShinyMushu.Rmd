...
#This did NOT work... is it because the ca_counties df is a shapefile and not a raster?
# Crop to the desired extent, then plot
bounds <- crop(ca_counties, extent(-125.5, -116.5, 31.7, 37.4))

#make the bounds polygon have wgs84
crs(bounds) <- crs(ca_counties)

proj4string(bounds) <- CRS(proj4string(ca_counties))
...
map_ca_hab <- tm_shape(coast_counties) +
  tm_fill("COUNTY", palette = "RdPu", alpha = 0.5)+
